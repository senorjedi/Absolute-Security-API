...
$LogFile = "$PSScriptRoot\UnfreezeDevice.log"
...
function search {
    ...
    try {
        ...
        $unfreeze = $resp.data.deviceFreezeStatus.passCode
         if (-not $unfreeze) {
            $unfreeze = $resp.data.deviceFreezeStatus.statuses.status.passCode
         }
         
        Add-Content -Path $LogFile -Value "$(Now) - Unfreeze code is $unfreeze"
        Write-Host "unfreeze code is $unfreeze"

        return $uuid
        
    }
    catch {
        Add-Content -Path $LogFile -Value "$(Now) - Failed the search call "
        Exit 1
    }
}
...
function unfreeze {
    ...
    Try {
        $Method = "POST"
        $Uri = "/v3/actions/freeze/remove-freeze"
        $QueryString = ""
        $Payload = @{
            data = @{
                deviceUids           = @("$uuid")
        ...
        Add-Content -Path $LogFile -Value "$(Now) - Could not do the unfreeze call"
        Exit 1
    }
}
...
Add-Content -Path $LogFile -Value "$(Now) - Starting the unfreeze call"
$unfreeze = unenroll -uuid $uuid
Add-Content -Path $LogFile -Value "$(Now) - Completed the unfreeze"
